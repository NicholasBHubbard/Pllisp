...
data ExprF
  ...
  | ExprCls Symbol [Symbol] [Symbol] [ClassMethod]  -- (CLASS Name (tyvars...) [REQUIRES (supers...)] methods...)
  | ExprInst Symbol Ty.Type [(Symbol, Expr)]  -- (INST ClassName %Type methods...)
  | ExprFFI Symbol (Maybe T.Text) [Ty.CType] Ty.CType  -- (FFI name [:link-name "..."] (c-param-types...) c-return-type)
  | ExprFFIStruct Symbol [(Symbol, Ty.CType)]  -- (FFI-STRUCT Name (field %CType)...)
  | ExprFFIVar Symbol (Maybe T.Text) [Ty.CType] Ty.CType  -- (FFI-VAR name [:link-name "..."] (fixed-params...) ret)
